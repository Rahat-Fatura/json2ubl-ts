...
export function serializeInvoice(
    ...
    parts.push(
        `${ind}${cbcTag("CustomizationID", UBL_CONSTANTS.customizationId)}`,
    );
    ...
    if (input.pricingCurrencyCode) {
        parts.push(
            `${ind}${cbcTag("PricingCurrencyCode", input.pricingCurrencyCode)}`,
        );
    }
    ...
    // 16. LineCountNumeric
    parts.push(
        `${ind}${cbcTag("LineCountNumeric", String(input.lines.length))}`,
    );
    ...
    if (input.paymentTerms) {
        ...
        if (input.paymentTerms.penaltySurchargePercent !== undefined) {
            parts.push(
                `${ind}  ${cbcTag("PenaltySurchargePercent", String(input.paymentTerms.penaltySurchargePercent))}`,
            );
        }
        if (input.paymentTerms.amount !== undefined) {
            parts.push(
                `${ind}  ${cbcTag("Amount", String(input.paymentTerms.amount))}`,
            );
        }
        parts.push(`${ind}</cac:PaymentTerms>`);
    }
